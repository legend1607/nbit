...
class NBITStar:
    ...
    def sample_from_env(self, c_best, batch_size, vertices=None, bias_ratio=0.3):
        ...
        if (
            ...
            pc = sample_fps_point_cloud(self.env, self.pc_n_points, oversample_scale=5)
        ...
        if len(selected_predicted) > 0:
            plt.scatter(selected_predicted[:,0], selected_predicted[:,1], c='blue', s=80, alpha=0.7, label='Predicted Top Points')
